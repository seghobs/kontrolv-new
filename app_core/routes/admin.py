...
from app_core.validators import is_valid_android_id, is_valid_device_id, is_valid_post_link, is_valid_username, normalize_username
...
def login():
    if request.method == "POST":
        from app_core.login_limits import client_key, reserve_attempt, clear_attempts
        key = client_key(request.remote_addr or "unknown", current_app.secret_key)
        retry_after = reserve_attempt(key)
        if retry_after:
            return render_template("admin_login.html", error=True,
                error_message=f"Çok fazla giriş denemesi. {retry_after // 60 + 1} dakika sonra tekrar deneyin."), 429, {"Retry-After": str(retry_after)}
        password = request.form.get("password")
        if password and ADMIN_PASSWORD and secrets.compare_digest(password.encode("utf-8"), ADMIN_PASSWORD.encode("utf-8")):
            clear_attempts(key)
...
def add_global_exemption_route():
    ...
    if success:
        day_text = f"{days} gün süreyle" if days > 0 else "süresiz olarak"
        return api_response(True, "ADDED", f"@{username} {day_text} muaf listeye eklendi")
...
def trigger_automation_route():
    ...
    job_id = enqueue('automation', {'thread_id': str(thread_id)})
    return api_response(True, "OK", "Otomasyon sıraya alındı.", extra={"job_id": job_id})
