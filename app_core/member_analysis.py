...
def start(source_id):
    ...
    with jobs.transaction() as conn:
        key='member-dual:'+source_id+':'+username+':'+date
...
def run(payload, progress):
    ...
    media=fetch_group_media_with_failover(payload['thread_id'],datetime.strptime(payload['date'],'%Y-%m-%d'),token_record=token,complete=True)
            ...
            async def check(post, likes):
                ...
                try:
                    ...
                    if likes:
                        ...
                        response=await fetch_comment_usernames_async(mid,token,session)
                        ...
                        elif response.get('ok'):row['state']='missing'
                except Exception:
                    row['state']='unknown'
                return row
            async def one(post):
    ...
    def report(items, mode):
        return {**payload,'check_likes':mode,'rows':items,'total':len(items),'counts':{s:sum(r['state']==s for r in items) for s in ('present','missing','unknown')}}
    if payload.get('dual_check'):
