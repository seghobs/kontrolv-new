(() => {
    ...
    const section=document.createElement('details');section.className='form-group';
    section.innerHTML='<summary>Mesajdan paylaşım bağlantılarını ayıkla</summary><textarea class="form-control" data-pasted-message placeholder="Bağlantıları içeren mesajı buraya yapıştırın"></textarea><button type="button" class="btn btn-secondary" data-extract>Bağlantıları forma aktar</button><p data-extract-status role="status"></p>';
    ...
    try{const last=localStorage.getItem('last-control');if(last&&/^[a-f0-9]{32}$/.test(last)){const link=document.createElement('a');link.href='/result/'+last;link.textContent='Son denetime geri dön';form.prepend(link);}}catch{}
