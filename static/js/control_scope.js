(() => {
    ...
    const utilities=document.getElementById('homeUtilities');if(utilities)utilities.append(section);else form.prepend(section);
    ...
    try{const last=localStorage.getItem('last-control');if(last&&/^[a-f0-9]{32}$/.test(last)){const link=document.createElement('a');link.href='/result/'+last;link.className='control-utility control-resume';link.innerHTML='<span class="control-utility-icon"><i class="fas fa-clock-rotate-left" aria-hidden="true"></i></span><span class="control-utility-copy"><strong>Son denetime geri dön</strong><small>Kaldığın raporu tekrar aç</small></span><i class="fas fa-arrow-right control-utility-arrow" aria-hidden="true"></i>';const resume=document.getElementById('homeResume');if(resume)resume.append(link);else form.prepend(link);}}catch{}
