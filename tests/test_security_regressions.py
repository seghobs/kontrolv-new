...
class SecurityTests(unittest.TestCase):
    ...
    def test_successful_comments_still_classify_members(self):
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(instagram, "get_post_details_async", new=AsyncMock(return_value={"like_count": 73, "comment_count": 24, "comment_count_verified": True})))
            stack.enter_context(patch.object(instagram, "fetch_comment_usernames_async", new=AsyncMock(return_value={"ok": True, "comments": [("alice", "Nice photo!")] + [(f"outside{i}", "Other comment") for i in range(23)]})))
